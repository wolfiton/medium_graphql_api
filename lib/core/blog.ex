defmodule MediumGraphqlApi.Blog do
  ...
  def create_comment(attrs = %{current_user: current_user, reply_user: reply_user}) do
    Multi.new()
    |> Multi.run(:create_comment, run_create_comment(attrs))
    |> Multi.run(:get_user, get_current_user(current_user))
    |> Multi.run(:get_reply, get_reply_user(reply_user))
  end
  ...
  defp get_current_user(current_user: current_user) do
    fn repo, _ ->
      {:ok, repo.one!(User, current_user)}
    end
  end

  defp get_reply_user(reply_user: reply_user) do
    fn repo, _ ->
      {:ok, repo.one!(Comment, reply_user)}
    end
  end
end
